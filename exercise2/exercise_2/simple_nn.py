...
class SimpleDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, split: Literal["train", "val"]):
        self.len = 4096 if split == "train" else 1024
        self.samples, self.labels = generate_toy_data(self.len)

    def __getitem__(self, idx):
        return np.expand_dims(self.samples[idx], axis=0), self.labels[idx]
...
class SimpleModel(torch.nn.Module):
    def __init__(self):
        ...
        self.conv1 = torch.nn.Conv3d(
            in_channels=1, out_channels=4, kernel_size=4, stride=3, padding=1
        )
        self.bn1 = torch.nn.BatchNorm3d(4)
        # TODO: Add conv2 and conv3 with the same parameters as conv1; also, add bn2 and bn3
        self.conv2 = torch.nn.Conv3d(
            in_channels=4, out_channels=8, kernel_size=4, stride=3, padding=1
        )
        self.bn2 = torch.nn.BatchNorm3d(8)
        self.conv3 = torch.nn.Conv3d(
            in_channels=8, out_channels=16, kernel_size=4, stride=3, padding=1
        )
        ...
        for i, batch in enumerate(train_dataloader):
            ...
            if iteration % config["validate_every_n"] == (
                ...
                for batch_val in val_dataloader:
                    ...
                    # TODO Move input_data and target_labels to device
                    input_data, target_labels = input_data.to(device), target_labels.to(
                        device
                    )
                ...
                print(
                    f"[{epoch:03d}/{i:05d}] val_loss: {loss_val / len(val_dataloader):.3f}, val_accuracy: {accuracy:.3f}%"
                )
                ...
                if accuracy > best_accuracy:
                    torch.save(
                        model.state_dict(),
                        f'exercise_2/runs/{config["experiment_name"]}/model_best.ckpt',
                    )
...
def main(config):
    ...
    Path(f'exercise_2/runs/{config["experiment_name"]}').mkdir(
