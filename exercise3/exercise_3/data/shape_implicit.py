...
class ShapeImplicit(torch.utils.data.Dataset):
    ...
    def __init__(self, num_sample_points, split):
        ...
        self.items = (
            Path(f"exercise_3/data/splits/sofas/{split}.txt").read_text().splitlines()
    ...
    def get_sdf_samples(self, path_to_sdf):
        ...
        samples_per_category = self.num_sample_points // 2
        pos_indices = torch.randperm(pos_tensor.shape[0])[:samples_per_category]
        neg_indices = torch.randperm(neg_tensor.shape[0])[:samples_per_category]

        pos_samples = pos_tensor[pos_indices]
        neg_samples = neg_tensor[neg_indices]
        combined_samples = torch.cat([pos_samples, neg_samples], dim=0)

        return combined_samples.float()
    ...
    def get_mesh(shape_id):
        ...
        """
        return trimesh.load(
            ShapeImplicit.dataset_path / shape_id / "mesh.obj", force="mesh"
        )
