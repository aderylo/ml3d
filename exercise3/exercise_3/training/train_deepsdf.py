...
def train(model, latent_vectors, train_dataloader, device, config):
    ...
    class SDFLoss(torch.nn.Module):
        def __init__(self, sigma=0.1):
            super().__init__()
            self.sigma = sigma

        def forward(self, pred, gt):
            pred_clamped = torch.clamp(pred, -self.sigma, self.sigma)
            gt_clamped = torch.clamp(gt, -self.sigma, self.sigma)
            loss = torch.abs(pred_clamped - gt_clamped).mean()
            return loss

    loss_criterion = SDFLoss()
    ...
    for epoch in range(config["max_epochs"]):
        ...
        for batch_idx, batch in enumerate(train_dataloader):
            ...
            batch_latent_vectors = (
                latent_vectors(batch["indices"])
                .unsqueeze(1)
                .expand(-1, batch["points"].shape[1], -1)
            )
            batch_latent_vectors = batch_latent_vectors.reshape(
                (num_points_per_batch, config["latent_code_length"])
            )
            ...
            decoder_input = torch.cat((batch_latent_vectors, points), dim=1)

            # TODO: perform forward pass
            predicted_sdf = model(decoder_input)
            ...
            code_regularization = (
                ...
                * config["lambda_code_regularization"]
            )
            if epoch > 100:
            ...
            if iteration % config["print_every_n"] == (config["print_every_n"] - 1):
                ...
                if train_loss < best_loss:
                    torch.save(
                        model.state_dict(),
                        f'exercise_3/runs/{config["experiment_name"]}/model_best.ckpt',
                    )
                    torch.save(
                        latent_vectors.state_dict(),
                        f'exercise_3/runs/{config["experiment_name"]}/latent_best.ckpt',
                    )
            ...
            if iteration % config["visualize_every_n"] == (
                config["visualize_every_n"] - 1
            ):
                # Set model to eval
                model.eval()
                latent_vectors_for_vis = latent_vectors(
                    torch.LongTensor(range(min(5, latent_vectors.num_embeddings))).to(
                        device
                    )
                )
                for latent_idx in range(latent_vectors_for_vis.shape[0]):
                    ...
                    evaluate_model_on_grid(
                        model,
                        latent_vectors_for_vis[latent_idx, :],
                        device,
                        64,
                        f'exercise_3/runs/{config["experiment_name"]}/meshes/{iteration:05d}_{latent_idx:03d}.obj',
                    )
...
def main(config):
    ...
    # create dataloaders
    train_dataset = ShapeImplicit(
        config["num_sample_points"], "train" if not config["is_overfit"] else "overfit"
    )
    ...
    latent_vectors = torch.nn.Embedding(
        len(train_dataset), config["latent_code_length"], max_norm=1.0
    )
    ...
    if config["resume_ckpt"] is not None:
        ...
        latent_vectors = torch.nn.Embedding.from_pretrained(
            torch.load(config["resume_ckpt"] + "_latent.ckpt", map_location="cpu")
        )
    ...
    Path(f'exercise_3/runs/{config["experiment_name"]}').mkdir(
